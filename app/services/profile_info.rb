class ProfileInfo < ApplicationService
    ...
    def get
        ...
        student_reviews = Review.where(student_teacher: true, connection_id: Connection.where(match_id: MatchRequest.where(student_id: @user.id)))
        teacher_reviews = Review.where(student_teacher: false,connection_id: Connection.where(match_id: MatchRequest.where(skill_class_id: SkillClass.where(teacher_id: @user.id))))
