...
export default class ClassData extends Component {
 ...
 render() {
  ...
  const durationOptions = durationDefaultOptions.concat([
   "customize",
  ]);
  ...
  const formIsValid = () => {
   return (
    ...
    (this.props.data.method ||
     this.props.data.regime == "physical") &&
    this.props.data.regime &&
    (this.props.data.location ||
     this.props.data.regime == "remote")
   );
  };
      ...
      {difficultyOptions.map((difficulty, i) => {
       return (
        <button
         key={i}
         onClick={() =>
          this.handleChange({ difficulty: difficulty })
         }
        >
         {difficulty.charAt(0).toUpperCase() +
          difficulty.slice(1) +
          (this.props.data.difficulty == difficulty
           ? " CHOSEN"
           : "")}
        </button>
       );
       ...
       if (durationDefaultOptions.includes(duration)) {
        ...
        return (
         <div key={i}>
          <button
           onClick={() =>
            this.handleChange({ duration: duration })
           }
          >
           {duration +
            " min" +
      ...
      {regimeOptions.map((regime, i) => {
       return (
        <button
         key={i}
         onClick={() =>
          this.handleChange({ regime: regime })
         }
        >
         {regime.charAt(0).toUpperCase() +
          regime.slice(1) +
          (this.props.data.regime == regime
           ? " CHOSEN"
           : "")}
        </button>
       );
