...
export default function SingleConnection(props) {
  ...
  return (
    <Layout current_user={props.user}>
      ...
      />
      <div>
        <img src={avatar(props.other_user.avatar.url)} alt="Avatar" />
        <p>{props.other_user.name}</p>
        <p>{props.other_user.username}</p>
        <button onClick={() => setPopupState(true)}>Bye!</button>
      </div>
      ...
      {popupState ? (
        <div>
          ...
          <button onClick={endConnectionHandler}>End Connection</button>
