class Api::ConnectionsController < ApplicationController
    ...
    def get_notifications
        ...
        u_n_json = @unread_notifications.as_json(only: [:id, :text, :notification_type, :created_at], include: {match: { only: [:id, :student_id], include: {connection: {only: [:id, :class_status]}}}})
        read_notifications = Notification.where(person_id: @current_user.id, read: true).order(created_at: :desc).as_json(only: [:id, :text, :notification_type, :created_at], include: {match: { only: [:id, :student_id], include: {connection: {only: [:id, :class_status]}}}})
